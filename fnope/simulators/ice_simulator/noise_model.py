...
def butter_lowpass_filter(x,y, cutoff, fs, order=5):
    ...
    b, a = butter_lowpass(cutoff, fs, order=order) #pyright: ignore
...
def twt_err(x, freq, PSD_log_mean, PSD_log_var):
    ...
    total_realizations = 4e-10 * torch.sum(realizations, axis=0) #pyright: ignore
...
def best_contour_diff_xs(x_true,true_layer,x_sim,sim_layers,method="MSE",layer_idx=None):
    ...
    return best_contour(true_layer,sim_layers,layer_mask,method,layer_idx)#pyright: ignore