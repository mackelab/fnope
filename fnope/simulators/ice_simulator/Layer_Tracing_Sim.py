...
import numpy as np
import pandas as pd
from fnope.simulators.ice_simulator.modelling_utils import regrid,regrid_all,trunc
...
class Geom():
    ...
    def fields_from_arrays(self,x_setup,base,surface,velocity,dQdx=None,dQdy=None):
        ...
        self.vxs = regrid(x_setup,velocity,np.linspace(self.x[0]-self.dx/2,self.x[-1]+self.dx/2,self.nx+1,endpoint=True)).reshape(self.nx+1,self.ny)#pyright: ignore
        self.vxs2 = regrid(x_setup,velocity,np.linspace(self.x[0],self.x[-1],self.nx+1)).reshape(self.nx+1,self.ny)#pyright: ignore

        self.ss = regrid(x_setup,surface,self.x).reshape(self.nx,self.ny)#pyright: ignore
        self.bs = regrid(x_setup,base,self.x).reshape(self.nx,self.ny)#pyright: ignore

        self.dQdx = regrid(x_setup,dQdx,self.x).reshape(self.nx,self.ny)#pyright: ignore
        self.dQdy = regrid(x_setup,dQdy,self.x).reshape(self.nx,self.ny)#pyright: ignore
...
def init_mb(geom,xmb,tmb=None,smb = None,bmb = None):
    ...
    if smb is not None and bmb is not None:
        smb_regrid =  regrid(xmb,smb,geom.x).reshape(geom.nx,geom.ny)#pyright: ignore
        bmb_regrid =  regrid(xmb,bmb,geom.x).reshape(geom.nx,geom.ny)#pyright: ignore
    #otherwise, figure out what is defined, and calculate the other using total mass balance
    else:
        assert tmb is not None
        tmb_regrid =  regrid(xmb,tmb,geom.x).reshape(geom.nx,geom.ny)#pyright: ignore
        if smb is not None:
            smb_regrid =  regrid(xmb,smb,geom.x).reshape(geom.nx,geom.ny)#pyright: ignore
    ...
    bmb_regrid = -bmb_regrid #the negative sign here is hidden, make bmb sign consistent and clearer #pyright: ignore
