...
from fnope.nets.fno_blocks import FNO1DBlock_DSE, FNO1DBlock_DSE_FixedContext
from fnope.nets.spectral_transforms import SpectralTransform,FFT1D,VFT1D
from fnope.nets.padding import get_padding
from fnope.nets.finite_nets import VectorFieldMLP
from fnope.nets.standardizing_net import FilterStandardizing, FiniteStandardizing, IdentityStandardizing
from fnope.flow_matching.base_distributions import WhiteNoise, FrequencyThresholdedGaussianProcess
...
class Velocity_Unified1D(nn.Module):
    def __init__(self,
                modes: int,
                in_channels: int,
                conv_channels: int,
                ...
                num_layers: int = 5,
                act = nn.GELU(),
                padding = {"type":"reflect","pad_length":20},
        ...
        if self.position_embedding_channels is not None:
            self.position_embedding_net = PositionEmbedding(domain_dim=1,hidden_dim=64, output_dim=position_embedding_channels, act=act)
            self.ctx_position_embedding_net = PositionEmbedding(domain_dim=1,hidden_dim=64, output_dim=position_embedding_channels, act=act)
        ...
        self.x_input_layer = nn.Conv1d(self.in_channels,self.conv_channels,1)
        self.ctx_input_layer = nn.Conv1d(self.ctx_in_channels,self.ctx_embedding_channels,1)
        #Make the ctx processing layer a bit more flexible by adding one FNO block with itself
        self.ctx_self_block = FNO1DBlock_DSE(in_channels=self.ctx_embedding_channels,
        ...
        for _ in range(num_layers):
            self.blocks.append(FNO1DBlock_DSE_FixedContext(in_channels=self.conv_channels,
        ...
        if x_finite_dim is not None:
            ...
            self.final_pos_proj = nn.Linear(self.position_embedding_channels, self.conv_channels) if self.position_embedding_channels is not None else None
            self.final_ctx_pos_proj = nn.Linear(self.position_embedding_channels, self.ctx_embedding_channels) if self.position_embedding_channels is not None else None
        ...
        self.f_last = nn.Conv1d(self.conv_channels, self.in_channels, 1)
    ...
    def get_trafo(self, batch_size:int,point_positions:Tensor, which:str ="x") -> SpectralTransform:
        ...
        point_positions: Tensor of shape (batch_size, n_points)
        ...
        if positions is not None and torch.equal(positions, point_positions) and self._batch_size == batch_size:
            ...
            trafo = VFT1D(batch_size, point_positions.shape[-1], self.modes, point_positions,device=point_positions.device)
            if which == "x":
    ...
    def process_points(self,
        ...
        point_positions: Tensor of shape (batch_size, n_points) or (n_points). If None, assume equispaced points on full domain.
        ...
        if point_positions is None:
            trafo = FFT1D(modes = self.modes, device = x.device)
            point_positions = torch.linspace(0, 1, x_n_points, device=x.device).unsqueeze(0).expand(batch_size, -1)
        else:
            assert point_positions.ndim == 1 or point_positions.ndim == 2, \
                f"point_positions must have shape (batch_size, n_points) or (n_points). Got {point_positions.shape}."
            n_points = point_positions.shape[-1]
            ...
            if point_positions.ndim ==1: 
                point_positions = point_positions.unsqueeze(0).expand(batch_size, -1) # (batch_size, n_points)
            elif point_positions.shape[0] == 1: # if point positions are fixed across batch, we repeat them
                point_positions = point_positions.expand(batch_size, -1) # (batch_size, n_points)
    ...
    def forward(self,
        ...
        # x has shape (batch, in_channels, nx_points)
        # ctx has shape (batch, ctx_in_channels, nctx_points)
        # t has shape (batch, 1)
        # x_finite has shape (batch, finite_dim)
        # point_positions has shape (batch_size, n_points) or (n_points)
        # ctx_point_positions has shape (batch_size, nctx_points) or (nctx_points)
        ...
        x = x.permute(0, 2, 1) # (batch_size, n_points, n_channels)
        ctx = self.ctx_input_layer(ctx)
        ctx = ctx.permute(0, 2, 1) # (batch_size, nctx_points, n_channels)
        ...
        if x_finite is not None:
            ...
            if p_embedding is not None and self.final_pos_proj is not None:
                final_p_embedding = self.final_pos_proj(self.act(p_embedding))
                x_spectral = torch.view_as_real(trafo.forward(x+ final_p_embedding)) #(batch_size, modes, conv_channels,2)
            else:
                x_spectral = torch.view_as_real(trafo.forward(x)) #(batch_size, modes, conv_channels,2)
            x_spectral = x_spectral.permute(0, 2 , 1, 3) # (batch_size, conv_channels, modes, 2)
            x_spectral = x_spectral.reshape(x_spectral.shape[0],  self.conv_channels, 2 * self.modes) # (batch_size, conv_channels, modes*2)

            if p_ctx_embedding is not None and self.final_ctx_pos_proj is not None:
                final_p_ctx_embedding = self.final_ctx_pos_proj(self.act(p_ctx_embedding))
                ctx_spectral = torch.view_as_real(ctx_trafo.forward(ctx + final_p_ctx_embedding))
            else:
                ctx_spectral = torch.view_as_real(ctx_trafo.forward(ctx))#(batch_size, modes, ctx_embedding_channels,2)
            ctx_spectral = ctx_spectral.permute(0, 2 , 1, 3) # (batch_size, conv_channels, modes, 2)
            ctx_spectral = ctx_spectral.reshape(ctx_spectral.shape[0],  self.ctx_embedding_channels, 2 * self.modes) # (batch_size, conv_channels, modes*2)
            x_finite_pred = self.finite_net(x_finite, x_spectral,ctx_spectral, t_embedding)
        
        x = x.permute(0, 2, 1) # (batch_size, conv_channels, n_points)
        x = self.f_last(x)
        if self.padding is not None:
            x, point_positions = self.padding.unpad(x, point_positions)
...
class FNOPE_1D(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        always_equispaced: If True, we always use equispaced points for the input and context. 
                           This will force the net to only use FFT, and never the VFT - will raise a warning if we pass in point_positions.
        ...
        super().__init__()
        self.modes = modes
        self.in_channels, self.nx_points, self.x_standardizing_net = self._read_continuous_input(x, simulation_grid=simulation_grid, standardize=True)
        ...
        if base_dist == "gp":
            ...
            self.base_dist = FrequencyThresholdedGaussianProcess(target_freq, self.in_channels,self.nx_points)
        elif base_dist == "wn":
            if x.ndim == 3:
                n_points = x.shape[-1]
            elif x.ndim == 4: 
            ...
            self.base_dist = WhiteNoise(self.in_channels, n_points)
        else:
        ...
        self.vel_net = Velocity_Unified1D(modes=self.modes,
    ...
    def _read_continuous_input(self,input:Tensor, simulation_grid:Optional[Tensor]=None, standardize:bool = True):
        ...
        Args:
        input: Tensor of shape (batch_size, in_channels, n_points)
        simulation_grid: Tensor of shape (n_points). If None, we assume an equispaced grid on the full domain.
        standardize: bool, whether to standardize the input or not.
        """

        assert input.ndim ==2 or input.ndim ==3, f"Input must be 2D or 3D tensor. Got {input.ndim}D tensor."
        if input.ndim == 2:
            warn("Input is 2D tensor. Adding batch dimension. Assuming input is of shape (batch, n_points) with 1 channel.")
            input = input.unsqueeze(1)

        in_channels, n_points = input.shape[1], input.shape[2]
        if standardize:
            standardizing_net = FilterStandardizing(input,simulation_grid,num_channels=in_channels,modes=self.modes,cutoff=False)
        else:
            standardizing_net = IdentityStandardizing()
        return in_channels, n_points, standardizing_net
    ...
    def forward(self,
        ...
        if x_batch_size == 1:
            x = x.expand(ctx_batch_size, -1, -1)
        elif ctx_batch_size == 1:
            ctx = ctx.expand(x_batch_size, -1, -1)
    ...
    def _vft_loss(self,
        ...
        if simulation_positions is None:
            warn("No simulation_positions provided. Assuming both `x` and `ctx` are equispaced." \
            "Using equispaced points for the simulation positions.")
            old_positions = torch.linspace(0, 1, x.shape[-1], device=x.device, dtype=x.dtype)
        else:
            old_positions = simulation_positions
        if ctx_simulation_positions is None:
            old_ctx_positions = old_positions
        else:
            old_ctx_positions = ctx_simulation_positions


        #expand batch size.
        old_positions.unsqueeze(0)
        old_positions = old_positions.expand(x.shape[0], -1) # (batch_size, n_points)
        old_ctx_positions.unsqueeze(0)
        old_ctx_positions = old_ctx_positions.expand(ctx.shape[0], -1) # (batch_size, n_points)
        ...
        with torch.no_grad():
            ...
            new_positions = old_positions[mask].view(batch_size, target_gridsize)
            ...
            if self.always_match_x_theta:
                ...
                mask.scatter_(1, indices, True)
                # Apply the mask to select new positions
                new_ctx_positions = old_ctx_positions[mask].view(batch_size, ctx_target_gridsize)
    ...
    def _fft_loss(self,
        ...
        # Currently, the shapes are:
        #x (batch_size, in_channels, n_points)
        #ctx (batch_size, ctx_in_channels, n_points)
        #x_finite (batch_size, finite_dim)

        epsilon_positions = torch.linspace(0, 1, x.shape[-1], device=x.device, dtype=x.dtype)
    ...
    def _unwrap_state(self, state:torch.Tensor):
        """
    ...
    def unnormalized_log_prob(self,
        ...
        if self.always_equispaced and (point_positions is not None or ctx_point_positions is not None):
            warn("The model was defined with `always_equispaced = True." \
                ...
                "The point positions will be ignored.")
            point_positions = None
        ...
        if x_finite is not None:
            ...
            state = x
        log_probs = self.flow(ctx=ctx,point_positions=point_positions,ctx_point_positions=ctx_point_positions,**kwargs).log_prob(state)
        return log_probs
    ...
    def sample(self,
        ...
        Args:
        num_samples: Number of samples to draw from the flow.
        ctx: Tensor of shape (ctx_in_channels, nctx_points)
        point_positions: Tensor of shape (n_points). If None, assume equispaced points on full domain.
        ...
        **kwargs: Additional arguments to pass to the flow.
        ...
        """
        assert ctx.ndim == 2, f"ctx must be 2D tensor of shape (ctx_in_channels, nctx_points). Got {ctx.ndim}D tensor."
        ctx = ctx.unsqueeze(0)
        ctx = ctx.expand(num_samples, -1, -1)
        ...
        if self.always_equispaced and (point_positions is not None or ctx_point_positions is not None):
            ...
            point_positions = None
            ctx_point_positions = None
        elif (not self.always_equispaced):
            if point_positions is None:
                warn("No point positions provided. Assuming equispaced points on full domain.")
                point_positions = torch.linspace(0, 1, self.nx_points, device=ctx.device, dtype=ctx.dtype)
            if ctx_point_positions is None:
                ctx_point_positions = point_positions
        ctx = self.ctx_standardizing_net.standardize(ctx,point_positions=ctx_point_positions)
        samples = self.flow(ctx=ctx,point_positions=point_positions,ctx_point_positions=ctx_point_positions,**kwargs).sample((num_samples,))
        if self.x_finite_dim is not None:
            samples, x_finite = self._unwrap_state(samples)
    ...
    def flow(self,
        ...
        if self.x_finite_dim is None:
            ...
            def combined_velocity(state, ctx, t,point_positions, ctx_point_positions):
                ...
                x, x_finite = self._unwrap_state(state) 
